  ...
  return (
    <div className="holder flex justify-center items-center px-16 py-20 text-xl font-bold text-center text-indigo-700 whitespace-nowrap shadow-lg bg-zinc-100 h-[800px] w-[800px] rounded-[90px] max-md:px-5">
      <div className="flex flex-col mt-1.5 max-w-full w-[531px]">
        ...
        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="shrink-0 mt-1.5 border border-indigo-700 border-solid bg-zinc-50 bg-opacity-0 h-[67px] rounded-[13px] max-md:max-w-full" />
        {errors.fields.email && (
        ...
        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="shrink-0 mt-4 border border-indigo-700 border-solid bg-zinc-50 bg-opacity-0 h-[67px] rounded-[13px] max-md:max-w-full" />
