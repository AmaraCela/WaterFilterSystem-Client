...
//USE THIS FUNCTION TO POPULATE FILE NAMES FROM DATABASE DATA INSTEAD
//JUST FOR TESTING

const generateRandomFileName = () => {
  const alphabet = "abcdefghijklmnopqrstuvwxyz";
  const fileNameLength = Math.floor(Math.random() * 10) + 5; // Random length between 5 and 14 characters
  let fileName = "";
  for (let i = 0; i < fileNameLength; i++) {
    const randomIndex = Math.floor(Math.random() * alphabet.length);
    fileName += alphabet[randomIndex];
  }
  fileName += ".xls"; // Adding file extension
  return fileName;
};
...
function HistoryUpload(props: MyComponentProps) {
  ...
  const fileNames = Array.from({ length: numberOfRows }, generateRandomFileName);
        ...
        <thead>
          <tr className="flex gap-5 justify-between max-md:flex-wrap max-md:max-w-full">
            <th className="flex flex-col">
              <div className="text-xl font-bold tracking-normal text-indigo-950">My uploads</div>
              <div className="mt-5 text-sm font-medium tracking-normal text-indigo-800">File Name</div>
            </th>
            <th className="self-end mt-11 text-sm font-medium tracking-normal text-indigo-800 max-md:mt-10">Date</th>
            <th className="flex flex-col self-start">
              {/* SortDropdown component */}
              <SortDropdown onSelect={(value: string) => {}} />
              <div className="self-center mt-4 text-sm font-medium tracking-normal text-center text-indigo-800">Status</div>
            </th>
          </tr>
        </thead>
        <tbody>
          ...
          {fileNames.map((fileName, index) => (
            <tr key={index} className="flex gap-5 items-center mt-2.5 text-xs font-medium tracking-normal max-md:flex-wrap max-md:max-w-full">
              <td className="flex-auto self-stretch my-auto text-sm tracking-normal text-zinc-800">{fileName}</td>
